...
function GetThumbprint($cert, [bool] $withSpaces) {
    ...
    if ($withSpaces) {
        $result = "";
        for ($th1i = 0; $th1i -lt $thumbprint.Length; $th1i++) {
            $c = $thumbprint[$th1i]
            if ($th1i -gt 0 -and $th1i % 2 -eq 0) {
                $result = $result + " $($c)"
            } else {
                $result = $result + "$($c)"
            }
...
if ($f_featurearray.Contains("sfstandalone")) {
    ...
    $versions = @{
       ...
       "9.0.1017.9590" = "https://download.microsoft.com/download/8/3/6/836E3E99-A300-4714-8278-96BC3E8B5528/9.0.1017.9590/Microsoft.Azure.ServiceFabric.WindowsServer.9.0.1017.9590.zip"
       "9.0.1048.9590" = "https://download.microsoft.com/download/8/3/6/836E3E99-A300-4714-8278-96BC3E8B5528/9.0.1048.9590/Microsoft.Azure.ServiceFabric.WindowsServer.9.0.1017.9590.zip"
    }
